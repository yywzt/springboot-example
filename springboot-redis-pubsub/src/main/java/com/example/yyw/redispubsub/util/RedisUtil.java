package com.example.yyw.redispubsub.util;
...
//        // 指定序列化输入的类型，类必须是非final修饰的，final修饰的类，比如String,Integer等会跑出异常
    ...
    public boolean hllAdd(String key,Object... value){
        try {
            Long aLong = redisTemplate.opsForHyperLogLog().add(key,value);
            if(aLong == 1){
                return true;
            }
            return false;
    ...
    public boolean hllDelete(String key){
        try {
    ...
    public Long hllUnion(String destination,String... sourceKey){
        try {
            Long size = redisTemplate.opsForHyperLogLog().union(destination,sourceKey);
            return size;
        } catch (Exception e) {
            e.printStackTrace();
            return null;
